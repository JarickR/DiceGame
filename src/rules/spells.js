...
export function resolveSpell(ctx, caster, face){
  ...
  const enemies = ctx.enemiesFn?.() ?? [];
  const firstEnemy = enemies.find(e => (e?.hp ?? 0) > 0) || enemies[0];

  if(t === 'attack'){
    const base = tier===1?2:tier===2?4:6;
    const target = ctx.chooseEnemy?.() || firstEnemy;
    ...
    if(caster.forbidConcentration){
      caster.hp = Math.max(0, (caster.hp||0) - 1);
      ctx.push?.(`${caster.name} cannot Concentrate (Barbarian).`);
      return;
    }
    ...
    enemies.forEach(e => ctx.applyDamage(caster, e, (base+barb)*mult, {type:'attack'}));
  }
  else if(t === 'fireball'){
    ...
    ctx.addPoison?.(ctx.chooseEnemy?.() || firstEnemy);
  }
  else if(t === 'bomb'){
    ctx.addBomb?.(ctx.chooseEnemy?.() || firstEnemy);
  }
  else if(t === 'upgrade'){
